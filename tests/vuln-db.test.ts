...
describe("where a scan looks for the database", () => {
  ...
  it("the static policy carries the directory and an explicit age limit, and a scan is given them as grype's own variables", () => {
    const policy = staticPolicyFromEnv({ HARNESS_HOME: "/h", HARNESS_VULN_DB_MAX_AGE_DAYS: "3" }, trustPolicyFromEnv({}), "linux", (p) => p === resolve("/h", "vuln-db"));
    expect(policy.vulnDbDir).toBe(resolve("/h", "vuln-db"));
    expect(policy.vulnDbMaxAgeDays).toBe(3);
    expect(staticPolicyFromEnv({ HARNESS_HOME: "/h" }, trustPolicyFromEnv({}), "linux", () => false)).not.toHaveProperty("vulnDbDir");
    expect(staticPolicyFromEnv({ HARNESS_HOME: "/h" }, trustPolicyFromEnv({}), "linux", () => false)).not.toHaveProperty("vulnDbMaxAgeDays");
  ...
  it("a scan that fails on the database says how to fetch it, in grype's own words plus the command; one that fails otherwise does not", () => {
    ...
    const scan = (stderr: string) => {
      ...
      const policy = { ...staticPolicyFromEnv({ HARNESS_SBOM_SOURCE: "generate" }, trustPolicyFromEnv({}), "linux", () => false), vulnDbDir: "/db" };
