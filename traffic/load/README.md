...
- **timing** (H4): the reader at a fixed request rate for a fixed window on
  each side, five runs per side, p95 compared with Mann–Whitney U in
  `src/compare/extra.ts`. Deterministic artefacts are compared once;
...
to catch a planted 30% SSR regression with `--runs 5` (three cannot reach alpha; the
mutant `slow-ssr` runs five).
