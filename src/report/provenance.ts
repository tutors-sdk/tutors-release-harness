 ...
 * here from a monorepo git ref, or pulled and judged unverified under
 * --allow-unsigned. A run over such a side is not evidence about what ships,
...
export function loudProvenance(report: RunReport): { sides: ("a" | "b")[]; text: string } | undefined {
  ...
  const sides = (["a", "b"] as const).filter((side) => Object.values(p[side]?.images ?? {}).some((info) => info.provenance === "pulled-unverified" || info.provenance === "built-from-ref"));
