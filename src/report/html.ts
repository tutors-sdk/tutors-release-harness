...
function provenanceBlock(report: RunReport): string {
  ...
  if (!p?.a && !p?.b) return "";
  const loud = (["a", "b"] as const).filter((side) => Object.values(p[side]?.images ?? {}).some((info) => info.provenance === "pulled-unverified" || info.provenance === "built-from-ref" || info.provenance === "cached"));
