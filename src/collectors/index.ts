...
export async function captureSide(spec: SideSpec, journeys: Journey[], opts: CaptureOptions): Promise<SideCapture> {
  ...
  const capture: SideCapture = { side: spec.name, harness: harnessInfo(), images: spec.images, ...(spec.provenance ? { provenance: spec.provenance } : {}), capturedAt: new Date().toISOString(), journeys: captured, metrics: { before, after }, logs, ...(spec.external ? { external: true } : {}), ...(spec.imageStatic ? { imageStatic: spec.imageStatic } : {}) };
  if (opts.load) {
